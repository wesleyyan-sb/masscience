...
function fitRollingTrend(measuredOnly, startDate, options = {}) {
  ...
  const weights = win.map((m, idx) => {
    let w = (m.isOutlier || m.isSodiumSpike || m.hasFluidTag) ? TREND.WEIGHT_OUTLIER : localOutlierWeight(win, idx);
...
function effectiveWeight(m) {
  if ((m.isOutlier || m.isSodiumSpike || m.hasFluidTag) && m.trendWeight != null) return m.trendWeight;
  return m.weight;
}
...
function buildTrendSeries(sorted, startDate, regression) {
  return sorted.map(m => {
    ...
    return {
      ...
      isOutlier: !!m.isOutlier || !!m.isSodiumSpike || !!m.hasFluidTag,
      isSodiumSpike: !!m.isSodiumSpike,
      hasFluidTag: !!m.hasFluidTag || !!m.isSodiumSpike,
      fluidTags: m.fluidTags || (m.isSodiumSpike ? ['sodium_spike'] : []),
...
export function calculateRateOfGain(series, measuredOnly, startDate, options = {}) {
  ...
  const weights = win.map((m, idx) => {
    let w = (m.isOutlier || m.isSodiumSpike || m.hasFluidTag) ? TREND.WEIGHT_OUTLIER : (m.isEstimated ? TREND.WEIGHT_ESTIMATED : localOutlierWeight(win, idx));
    if (phaseStart && m.date < phaseStart) {
      const daysPrior = Math.max(1, daysBetween(m.date, phaseStart));
      w *= clamp(Math.pow(0.5, daysPrior / 2.0), 0.04, 0.25);
    }
